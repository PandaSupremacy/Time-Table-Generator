...
def repairLostSLCT(chromosome):
    ...
    chromosome = weektosubs(chromosome)
    empty_indices = set(i for i, _ in enumerate(chromosome) if not _)
    while courseCred:
        rsub = random.choice(list(courseCred.keys()))
        fitness_indices = []
        for ind, _ in enumerate(chromosome):
            if ind in empty_indices:
                chromosome[ind] = rsub
                fitness_indices.append((fitnessFunction(substoweek(chromosome)), ind))
                chromosome[ind] = ''
        maxfit, maxind = max(fitness_indices)
        chromosome[maxind] = rsub
        courseCred[rsub] -= 1
        if courseCred[rsub] == 0:
            del courseCred[rsub]
        empty_indices.remove(maxind)
